...
foreach ($cars as $c) {
    $out[] = [
        ...
        'specs'  => [
            ...
            'Torque'         => $c['torque_nm'] ? $c['torque_nm'] . ' Nm' : '',
            'Transmission'   => $c['transmission'],
            'Drive type'     => $c['drive_type'],
            '0–100 km/h'     => $c['acceleration_s'] ? $c['acceleration_s'] . ' s' : '',
            'Top speed'      => $c['top_speed_kmh'] ? $c['top_speed_kmh'] . ' km/h' : '',
            'City economy'   => $c['mileage_city_kml'] ? $c['mileage_city_kml'] . ' km/l' : '',
            'Highway economy'=> $c['mileage_highway_kml'] ? $c['mileage_highway_kml'] . ' km/l' : '',
            'Battery'        => $c['battery_kwh'] ? $c['battery_kwh'] . ' kWh' : '',
            'Electric range' => $c['range_km'] ? $c['range_km'] . ' km' : '',
            'Length'         => $c['length_mm'] ? $c['length_mm'] . ' mm' : '',
            'Width'          => $c['width_mm'] ? $c['width_mm'] . ' mm' : '',
            'Height'         => $c['height_mm'] ? $c['height_mm'] . ' mm' : '',
            'Wheelbase'      => $c['wheelbase_mm'] ? $c['wheelbase_mm'] . ' mm' : '',
            'Ground clearance' => $c['ground_clearance_mm'] ? $c['ground_clearance_mm'] . ' mm' : '',
            'Curb weight'    => $c['curb_weight_kg'] ? $c['curb_weight_kg'] . ' kg' : '',
            'Boot space'     => $c['boot_space_l'] ? $c['boot_space_l'] . ' L' : '',
